import type { RecoverySystem, RecoveryMode } from "@/lib/types";

/**
 * Single source of truth for per-system metadata and the science behind the
 * scoring. Consumed by:
 *   - the scoring engine (`src/stressors/scoring.ts`) — label, icon, recovery
 *     window, and `scienceFor()` for each SystemScore's fact/cite
 *   - the evidence deep-dive (`evidence/systems-science.ts`) — full cards
 *   - the Tether judge page (`tether/tether-data.ts`) — the fan-mode citation
 *
 * Pure data + selectors, no React. A fact, cite, icon, or recovery window is
...
export interface SystemDef {
  ...
  /** Display label, e.g. "Brain / Cognition". */
  label: string;
  icon: string;
  /** Accent hex used by the evidence cards. */
  accent: string;
  /** Base deterministic recovery window in hours. */
  recoveryWindowHrs: number;
  /** Default (personal / football) science. */
  base: ScienceEntry;
  /** Per-mode science overrides, e.g. fan-mode emotional-stress science. */
  overrides?: Partial<Record<RecoveryMode, ScienceEntry>>;
}

// ─── Canonical per-system definitions (display order) ────────────────────────
...
export const SYSTEMS: SystemDef[] = [
  ...
  },
  {
    ...
    base: {
      ...
      expanded:
        "The brain is the most stressor-sensitive system in the model. Alcohol — especially spirits and cocktails (1.3–1.4× modifiers) — directly suppresses prefrontal activity. Sleep under 6 hours adds a compounding penalty of 4 points per missing hour via circadian misalignment. Concussion events carry 50-point base hits, the largest single-stressor impact in the engine. The brain's recovery window is 24 hours, reflecting the time required for glymphatic clearance and neurotransmitter rebalance.",
      stressors: [
  ...
  },
  {
    ...
    base: {
      ...
      expanded:
        "Muscle recovery is primarily CNS-driven. The scoring engine assigns 40 base points to training stressors, weighted by body area (legs/full-body = 1.0, upper = 0.5, mobility = −0.5) and intensity (destroyed = 1.2×, hard = 0.85×, easy = 0.4×). Match minutes in football mode add 35 points weighted by playing time. Alcohol further suppresses the mTOR pathway, reducing protein synthesis. The muscular system has the longest recovery window at 48 hours — consistent with literature on full glycogen depletion and muscle fibre repair cycles.",
      stressors: [
...
const BY_SYSTEM = Object.fromEntries(SYSTEMS.map((s) => [s.system, s])) as Record<
  RecoverySystem,
  SystemDef
>;

/** System metadata for the scoring engine (label, icon, recovery window). */
export const SYSTEM_META: Record<
  RecoverySystem,
  { label: string; icon: string; baseWindowHrs: number }
> = Object.fromEntries(
  SYSTEMS.map((s) => [s.system, { label: s.label, icon: s.icon, baseWindowHrs: s.recoveryWindowHrs }]),
) as Record<RecoverySystem, { label: string; icon: string; baseWindowHrs: number }>;

/** The science entry for a system in a given mode (mode override, else base). */
export function scienceEntry(system: RecoverySystem, mode: RecoveryMode): ScienceEntry {
  const def = BY_SYSTEM[system];
  return def.overrides?.[mode] ?? def.base;
}

/** Just the fact + citation — what the scoring engine attaches to a SystemScore. */
export function scienceFor(
  system: RecoverySystem,
  mode: RecoveryMode,
): { fact: string; cite: string } {
  const e = scienceEntry(system, mode);
  return { fact: e.fact, cite: e.cite };
}
