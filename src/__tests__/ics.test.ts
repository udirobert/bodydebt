...
describe("generateRecoveryIcs", () => {
  ...
  describe("required iCalendar properties", () => {
    ...
    it("includes a PRODID with the app identifier", () => {
      const ics = generateRecoveryIcs(VALID_INPUT);
      expect(ics).toContain("PRODID:-//Orbura//Recovery Calendar//EN");
    });
  ...
  describe("event timing", () => {
    ...
    it("includes a UID with the correct format", () => {
      ...
      expect(ics).toMatch(/^UID:orbura-recovery-\d{8}T\d{6}@orbura\.famile\.xyz$/m);
    });
  ...
  describe("event content", () => {
    it("sets SUMMARY to 'Recovery Window — <appName>'", () => {
      const ics = generateRecoveryIcs({ ...VALID_INPUT, appName: "Orbura" });
      // SUMMARY is short enough to not be folded
      expect(ics).toMatch(/^SUMMARY:Recovery Window — Orbura$/m);
    });

    it("uses default appName 'Orbura' when not provided", () => {
      const ics = generateRecoveryIcs(VALID_INPUT);
      expect(ics).toMatch(/^SUMMARY:Recovery Window — Orbura$/m);
    });
    ...
    it("includes 'Generated by orbura.famile.xyz' in the DESCRIPTION", () => {
      const ics = generateRecoveryIcs(VALID_INPUT);
      expect(ics).toContain("orbura.famile.xyz");
    });
  });
